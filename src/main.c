...
extern void rb_parser_config_default_initialize(rb_parser_config_t *config);
...
print_array(FILE *fp, int indent, const NODE *node)
{
    ...
    dump_node(node->nd_head, indent+1);
...
dump_node(const NODE *node, const int indent)
{
    ...
    switch (type) {
      case NODE_SCOPE:
        PRINT(stderr, "SCOPE:\n");
        dump_node(node->nd_args, indent+1);
        dump_node(node->nd_body, indent+1);
        return;
      case NODE_OPCALL:
        PRINT(stderr, "OPCALL:\n");
        PRINT_ID(stderr, node->nd_mid);
        dump_node(node->nd_recv, indent+1);
        dump_node(node->nd_args, indent+1);
        return;        
      case NODE_LIT:
        PRINT(stderr, "LIT:\n");
        PRINT_LIT(stderr, node->nd_lit);
        return;        
      case NODE_LIST:
        PRINT(stderr, "LIST:\n");
...
main()
{
    ...
    rb_parser_config_default_initialize(config);
